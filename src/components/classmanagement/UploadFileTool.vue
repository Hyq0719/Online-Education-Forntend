    ...
    export default {
        ...
        data () {
            return {
                ...
                fileBase64List : [] ,
                acceptObj : {
                    flv: { type : "flv" , icon : "iconfont icon-word" , color : "#0061b2" , tagType : "" } ,
                    mp4: { type : "Mp4" , icon : "iconfont icon-word" , color : "#0061b2" , tagType : "" } ,
        ...
        methods : {
            beforeUploadChange ( file ) {
                ...
                if ( isIntercept ) {     //!that.acceptObj.hasOwnProperty ( isType )报错
                    that.eleNotify ( `${file.name + "文件类型不正确"}` , 3 )
                } else if ( file.size >= that.fileSize ) {
                    let sizeText = bytesToSize ( file.size )
                    let regSizeText = bytesToSize ( that.fileSize )
                    that.eleNotify ( `${file.name + "图片超过指定大小"}：${sizeText}>${regSizeText}` , 3 )
                } else {
